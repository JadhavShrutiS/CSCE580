...
class MCAgent:
    ...
    def solve(self, initial_missionaries, initial_cannibals):
        #my code modified using the provided
        ...
        def dfs(): #using a depth first search

            initial_state = States(initial_missionaries, initial_cannibals, 0, 0, "left")  # root
            if initial_state.goal_state():
                return initial_state
    
            stack = [] # allows us to traverse the end of the node for a tree (depth of the tree)
            ...
            while stack:
                ...
                exploredStruc.append(node)
                node_children = successors(node)
                
                for child in node_children:
                    if (child not in exploredStruc) and (child not in stack):
                        stack.append(child)
